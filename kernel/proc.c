...
int nextpid = 1;
unsigned int seed = 111;    // Seed for srand(), incremented by 1 after every scheduler loop.
...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->tickets = 1;  // Set default ticket value of 1 to new process.
  release(&ptable.lock);
...
fork(void)
{
  ...
  np->sz = proc->sz;
  np->parent = proc;
  np->tickets = proc->tickets;  // Set child tickets to parent tickets
  *np->tf = *proc->tf;
